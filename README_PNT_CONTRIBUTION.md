...
1. **WeierstrassProduct.lean**: 
   ...
   - Proves the key cubic tail bound \(\|\log(1-z) + z + z^2/2\| \le \|z\|^3/(1-\|z\|)\) on \(\|z\|<1\).

2. **Determinant.lean**:
   - Defines `det2_AF(s)` as an Euler product over primes with local factor \((1-\lambda)\exp(\lambda+\lambda^2/2)\) for \(\lambda=p^{-s}\).
...
4. **GammaBounds.lean**:
   - Provides `BoundedFGammaPrimeOnStrip`, a Prop-level interface for uniform bounds on the Archimedean factor \(\Gamma(s/2)\pi^{-s/2}\), plus an explicit nonnegative constant constructor.
